...
int main() {
	...
	if (Quick_Sort_PISTRO(array, 0, len - 1)) {
		printf_s("sorting error");
	}
...
int Quick_Sort_PISTRO(unsigned int* data, int left, int right) {
	while (right - left >= 10) {
		...
		if (partition_idx - left < right - partition_idx) {
			Quick_Sort_PISTRO(data, left, partition_idx - 1);
			left = partition_idx + 1;
		}
		else {
			Quick_Sort_PISTRO(data, partition_idx + 1, right);
